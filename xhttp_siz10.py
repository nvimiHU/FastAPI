# xhttp_siz10.py
# XHTTP Ultra Transport — فقط XHTTP بدون وابستگی به VLESS
...
from main import stats, connections, logger
...
TARGET_HOST = os.environ.get("XHTTP_TARGET_HOST", "localhost")
TARGET_PORT = int(os.environ.get("XHTTP_TARGET_PORT", "443"))
...
async def _open_tcp_connection(data: bytes):
    reader, writer = await asyncio.wait_for(
        asyncio.open_connection(TARGET_HOST, TARGET_PORT), timeout=TCP_CONNECT_TIMEOUT
    )
    _tune_socket(writer)
    if data:
        writer.write(data)
        await writer.drain()
    return reader, writer, TARGET_HOST, TARGET_PORT
...
async def _pump_tcp_to_queue(session_id: str, reader: asyncio.StreamReader, down_q: asyncio.Queue):
    first = True
    try:
        while True:
...
async def _open_tcp_for_session(session_id: str, sess: dict, first_chunk: bytes):
    reader, writer, address, port = await _open_tcp_connection(first_chunk)
